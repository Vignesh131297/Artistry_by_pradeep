...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .page{width:780px;max-width:none;margin:0 auto}
  .page-viewport{width:100%;max-width:780px;margin:0 auto;overflow:hidden}
  .page-view{width:780px;transform-origin:top left}
...
async function savePdfBlob(blob, filename, allowShare=true){
  const file = new File([blob], filename, {type:'application/pdf'});

  if(allowShare && isMobileDevice() && await canSharePdfFile(file)){
    try{
...
async function saveAsPdf(){
  ...
  try{
    ...
    const result = await savePdfBlob(blob, PDF_FILENAME, false);
...
<script>
  function fitPreview(){
    ...
    if(!viewport||!pageView||!page) return;
    const scale=Math.min(1, viewport.clientWidth/page.offsetWidth);
    pageView.style.transform='scale('+scale+')';
